...
static void swapchain_create_render_pass(swapchain_t *swapchain);
...
static void swapchain_destroy_render_pass(swapchain_t *swapchain);
...
static void swapchain_create_render_pass(swapchain_t *swapchain) {
  ...
  VULKAN_CHECK(vkCreateRenderPass(swapchain->context->device, &render_pass_create_info, 0, &swapchain->render_pass));
}
static void swapchain_create_color_images(swapchain_t *swapchain) {
...
static void swapchain_create_depth_images(swapchain_t *swapchain) {
  ...
  swapchain->depth_image_device_memory = (VkDeviceMemory *)heap_alloc(sizeof(VkDeviceMemory) * swapchain->image_count, 0, 0);
  ...
  while (image_index < image_count) {
    ...
    VULKAN_CHECK(vkAllocateMemory(swapchain->context->device, &memory_allocate_info, 0, &swapchain->depth_image_device_memory[image_index]));
    VULKAN_CHECK(vkBindImageMemory(swapchain->context->device, swapchain->depth_image[image_index], swapchain->depth_image_device_memory[image_index], 0));
...
static void swapchain_create_frame_buffer(swapchain_t *swapchain) {
  swapchain->frame_buffers = (VkFramebuffer *)heap_alloc(sizeof(VkFramebuffer) * swapchain->image_count, 0, 0);
...
static void swapchain_destroy_render_pass(swapchain_t *swapchain) {
  vkDestroyRenderPass(swapchain->context->device, swapchain->render_pass, 0);
}
static void swapchain_destroy_color_images(swapchain_t *swapchain) {
  uint64_t image_index = 0;
  uint64_t image_count = swapchain->image_count;
...
static void swapchain_destroy_depth_images(swapchain_t *swapchain) {
  ...
  while (image_index < image_count) {
    ...
    vkFreeMemory(swapchain->context->device, swapchain->depth_image_device_memory[image_index], 0);
  ...
  heap_free(swapchain->depth_image_device_memory);
...
static void swapchain_destroy_frame_buffer(swapchain_t *swapchain) {
  ...
  while (image_index < image_count) {
    ...
    vkDestroyFramebuffer(swapchain->context->device, swapchain->frame_buffers[image_index], 0);

    image_index++;
  }

  heap_free(swapchain->frame_buffers);
}
