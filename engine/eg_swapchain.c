...
void swapchain_create(void) {
  ...
  g_swapchain_image_count = swapchain_asset.image_count;
  g_swapchain_depth_format = swapchain_asset.depth_format;

  g_swapchain_image_count = MAX(g_swapchain_image_count, g_context_surface_capabilities.minImageCount);
  g_swapchain_image_count = MIN(g_swapchain_image_count, g_context_surface_capabilities.maxImageCount);

  g_swapchain_min_image_count = g_context_surface_capabilities.minImageCount;

  VkSwapchainCreateInfoKHR swapchain_create_info = {0};
  swapchain_create_info.sType = VK_STRUCTURE_TYPE_SWAPCHAIN_CREATE_INFO_KHR;
  swapchain_create_info.surface = g_context_surface;
  swapchain_create_info.minImageCount = g_swapchain_image_count;
...
static void swapchain_create_render_pass(void) {
  ...
  depth_attachment_description.format = g_swapchain_depth_format;
...
static void swapchain_create_color_images(void) {
  s_swapchain_color_image = (VkImage *)heap_alloc(sizeof(VkImage) * g_swapchain_image_count, 0, 0);
  s_swapchain_color_image_view = (VkImageView *)heap_alloc(sizeof(VkImageView) * g_swapchain_image_count, 0, 0);

  VULKAN_CHECK(vkGetSwapchainImagesKHR(g_context_device, g_swapchain, &g_swapchain_image_count, s_swapchain_color_image));

  uint32_t image_index = 0;
  while (image_index < g_swapchain_image_count) {
...
static void swapchain_create_depth_images(void) {
  s_swapchain_depth_image = (VkImage *)heap_alloc(sizeof(VkImage) * g_swapchain_image_count, 0, 0);
  s_swapchain_depth_image_device_memory = (VkDeviceMemory *)heap_alloc(sizeof(VkDeviceMemory) * g_swapchain_image_count, 0, 0);
  s_swapchain_depth_image_view = (VkImageView *)heap_alloc(sizeof(VkImageView) * g_swapchain_image_count, 0, 0);
  ...
  while (image_index < g_swapchain_image_count) {
    ...
    image_create_info.format = g_swapchain_depth_format;
    ...
    image_view_create_info.viewType = VK_IMAGE_VIEW_TYPE_2D;
    image_view_create_info.format = g_swapchain_depth_format;
...
static void swapchain_create_frame_buffer(void) {
  g_swapchain_frame_buffers = (VkFramebuffer *)heap_alloc(sizeof(VkFramebuffer) * g_swapchain_image_count, 0, 0);
