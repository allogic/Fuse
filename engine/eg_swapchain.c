...
void swapchain_create(void) {
  ...
  g_globals.swapchain_image_count = MAX(g_globals.swapchain_image_count, g_globals.context_surface_capabilities.minImageCount);
  g_globals.swapchain_image_count = MIN(g_globals.swapchain_image_count, g_globals.context_surface_capabilities.maxImageCount);

  g_globals.swapchain_min_image_count = g_globals.context_surface_capabilities.minImageCount;

  VkSwapchainCreateInfoKHR swapchain_create_info = {0};
  swapchain_create_info.sType = VK_STRUCTURE_TYPE_SWAPCHAIN_CREATE_INFO_KHR;
  swapchain_create_info.surface = g_globals.context_surface;
  swapchain_create_info.minImageCount = g_globals.swapchain_image_count;
  swapchain_create_info.imageFormat = g_globals.context_prefered_surface_format.format;
  swapchain_create_info.imageColorSpace = g_globals.context_prefered_surface_format.colorSpace;
  swapchain_create_info.imageExtent.width = g_globals.context_surface_width;
  swapchain_create_info.imageExtent.height = g_globals.context_surface_height;
  swapchain_create_info.imageArrayLayers = 1;
  swapchain_create_info.imageUsage = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT;
  swapchain_create_info.preTransform = g_globals.context_surface_capabilities.currentTransform;
  swapchain_create_info.compositeAlpha = VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR;
  swapchain_create_info.presentMode = g_globals.context_prefered_present_mode;
  swapchain_create_info.clipped = 1;
  swapchain_create_info.oldSwapchain = 0;

  int32_t queue_families[2] = {g_globals.context_graphic_queue_index, g_globals.context_present_queue_index};

  if (g_globals.context_graphic_queue_index == g_globals.context_present_queue_index) {
  ...
  VULKAN_CHECK(vkCreateSwapchainKHR(g_globals.context_device, &swapchain_create_info, 0, &g_globals.swapchain));
...
void swapchain_destroy(void) {
  ...
  vkDestroySwapchainKHR(g_globals.context_device, g_globals.swapchain, 0);
}

static void swapchain_create_render_pass(void) {
  VkAttachmentDescription color_attachment_description = {0};
  color_attachment_description.format = g_globals.context_prefered_surface_format.format;
  ...
  depth_attachment_description.format = g_globals.swapchain_depth_format;
...
static void swapchain_create_color_images(void) {
  ...
  while (image_index < g_globals.swapchain_image_count) {
    ...
    image_view_create_info.format = g_globals.context_prefered_surface_format.format;
    ...
    VULKAN_CHECK(vkCreateImageView(g_globals.context_device, &image_view_create_info, 0, &s_swapchain_color_image_view[image_index]));
...
static void swapchain_create_depth_images(void) {
  ...
  while (image_index < g_globals.swapchain_image_count) {
    ...
    image_create_info.extent.width = g_globals.context_surface_width;
    image_create_info.extent.height = g_globals.context_surface_height;
    image_create_info.extent.depth = 1;
    image_create_info.mipLevels = 1;
    image_create_info.arrayLayers = 1;
    image_create_info.format = g_globals.swapchain_depth_format;
    ...
    VULKAN_CHECK(vkCreateImage(g_globals.context_device, &image_create_info, 0, &s_swapchain_depth_image[image_index]));

    VkMemoryRequirements memory_requirements = {0};

    vkGetImageMemoryRequirements(g_globals.context_device, s_swapchain_depth_image[image_index], &memory_requirements);
    ...
    VULKAN_CHECK(vkAllocateMemory(g_globals.context_device, &memory_allocate_info, 0, &s_swapchain_depth_image_device_memory[image_index]));
    VULKAN_CHECK(vkBindImageMemory(g_globals.context_device, s_swapchain_depth_image[image_index], s_swapchain_depth_image_device_memory[image_index], 0));

    VkImageViewCreateInfo image_view_create_info = {0};
    image_view_create_info.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
    image_view_create_info.image = s_swapchain_depth_image[image_index];
    image_view_create_info.viewType = VK_IMAGE_VIEW_TYPE_2D;
    image_view_create_info.format = g_globals.swapchain_depth_format;
    ...
    VULKAN_CHECK(vkCreateImageView(g_globals.context_device, &image_view_create_info, 0, &s_swapchain_depth_image_view[image_index]));
...
static void swapchain_create_frame_buffer(void) {
  ...
  while (image_index < g_globals.swapchain_image_count) {
    ...
    frame_buffer_create_info.renderPass = g_globals.swapchain_render_pass;
    ...
    frame_buffer_create_info.width = g_globals.context_surface_width;
    frame_buffer_create_info.height = g_globals.context_surface_height;
    frame_buffer_create_info.layers = 1;

    VULKAN_CHECK(vkCreateFramebuffer(g_globals.context_device, &frame_buffer_create_info, 0, &g_globals.swapchain_frame_buffers[image_index]));
...
static void swapchain_destroy_render_pass(void) {
  vkDestroyRenderPass(g_globals.context_device, g_globals.swapchain_render_pass, 0);
}
static void swapchain_destroy_color_images(void) {
  uint32_t image_index = 0;
  while (image_index < g_globals.swapchain_image_count) {
    vkDestroyImageView(g_globals.context_device, s_swapchain_color_image_view[image_index], 0);
...
static void swapchain_destroy_depth_images(void) {
  uint32_t image_index = 0;
  while (image_index < g_globals.swapchain_image_count) {
    vkDestroyImageView(g_globals.context_device, s_swapchain_depth_image_view[image_index], 0);
    vkFreeMemory(g_globals.context_device, s_swapchain_depth_image_device_memory[image_index], 0);
    vkDestroyImage(g_globals.context_device, s_swapchain_depth_image[image_index], 0);
...
static void swapchain_destroy_frame_buffer(void) {
  uint32_t image_index = 0;
  while (image_index < g_globals.swapchain_image_count) {
    vkDestroyFramebuffer(g_globals.context_device, g_globals.swapchain_frame_buffers[image_index], 0);

    image_index++;
  }

  heap_free(g_globals.swapchain_frame_buffers);
}
