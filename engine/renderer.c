...
void renderer_draw_debug_line(vector3_t from, vector3_t to, vector4_t color) {
  if (g_renderer_enable_debug) {
    ...
    if (debug_pipeline) {
      ...
      vector4_t *colors = (vector4_t *)graphic_pipeline_vertex_buffer(debug_pipeline, s_renderer_frame_index, 1);
...
static void renderer_record_graphic_commands(void) {
  ...
  UI_DRAW(s_renderer_graphic_command_buffers[s_renderer_frame_index]);
