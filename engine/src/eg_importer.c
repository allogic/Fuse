...
static void eg_importer_process_pipeline_vertex_input_bindings(eg_pipeline_asset_t *pipeline_asset, SpvReflectShaderModule *shader_module);
static void eg_importer_process_pipeline_descriptor_bindings(eg_pipeline_asset_t *pipeline_asset, SpvReflectShaderModule *shader_module, VkShaderStageFlags stage_flag);

void eg_importer_import_default_assets(void) {
  eg_graphic_pipeline_import_settings_t debug_line_import_settings = {0};

  strcpy(debug_line_import_settings.pipeline_name, "debug_line");
  strcpy(debug_line_import_settings.vertex_shader_file_path, EG_ASSET_ROOT_DIR "\\shader\\debug_line_vert.spv");
  ...
  eg_graphic_pipeline_import_settings_t principled_brdf_import_settings = {0};

  strcpy(principled_brdf_import_settings.pipeline_name, "principled_brdf");
  strcpy(principled_brdf_import_settings.vertex_shader_file_path, EG_ASSET_ROOT_DIR "\\shader\\principled_brdf_vert.spv");
  ...
  eg_graphic_pipeline_import_settings_t terrain_import_settings = {0};

  strcpy(terrain_import_settings.pipeline_name, "terrain");
  strcpy(terrain_import_settings.vertex_shader_file_path, EG_ASSET_ROOT_DIR "\\shader\\environment_terrain_vert.spv");
  ...
  eg_model_import_settings_t kalista_import_settings = {0};

  strcpy(kalista_import_settings.model_name, "kalista");
  strcpy(kalista_import_settings.model_file_path, EG_ASSET_ROOT_DIR "\\model\\kalista.glb");

  eg_importer_import_model(&kalista_import_settings);
}
...
void eg_importer_import_graphic_pipeline(eg_graphic_pipeline_import_settings_t *import_settings) {
  ...
  eg_filesys_load_binary(&vertex_shader_bytes, &vertex_shader_size, import_settings->vertex_shader_file_path);
  eg_filesys_load_binary(&fragment_shader_bytes, &fragment_shader_size, import_settings->fragment_shader_file_path);

  EG_SPIRV_CHECK(spvReflectCreateShaderModule(vertex_shader_size, vertex_shader_bytes, &reflect_vertex_shader_module));
  EG_SPIRV_CHECK(spvReflectCreateShaderModule(fragment_shader_size, fragment_shader_bytes, &reflect_fragment_shader_module));

  eg_pipeline_asset_t pipeline_asset = {0};

  strcpy(pipeline_asset.name, import_settings->pipeline_name);
  ...
  eg_importer_process_pipeline_descriptor_bindings(&pipeline_asset, &reflect_vertex_shader_module, VK_SHADER_STAGE_VERTEX_BIT);
  eg_importer_process_pipeline_descriptor_bindings(&pipeline_asset, &reflect_fragment_shader_module, VK_SHADER_STAGE_FRAGMENT_BIT);
...
void eg_importer_import_compute_pipeline(eg_compute_pipeline_import_settings_t *import_settings) {
  ...
  eg_filesys_load_binary(&compute_shader_bytes, &compute_shader_size, import_settings->compute_shader_file_path);

  EG_SPIRV_CHECK(spvReflectCreateShaderModule(compute_shader_size, compute_shader_bytes, &reflect_compute_shader_module));
...
void eg_importer_import_model(eg_model_import_settings_t *import_settings) {
  ...
  eg_filesys_load_binary(&model_bytes, &model_size, import_settings->model_file_path);

  cgltf_options gltf_options = {0};

  cgltf_data *gltf_data = 0;

  EG_GLTF_CHECK(cgltf_parse(&gltf_options, model_bytes, model_size, &gltf_data));
  EG_GLTF_CHECK(cgltf_load_buffers(&gltf_options, gltf_data, import_settings->model_file_path));

  eg_model_asset_t model_asset = {0};

  strcpy(model_asset.name, import_settings->model_name);

  eg_database_store_model_asset(&model_asset);

  eg_model_resource_t model_resource = {0};

  model_resource.model_asset_id = model_asset.id;
  strcpy(model_resource.model_file_path, import_settings->model_file_path);

  eg_database_store_model_resource(&model_resource);
  ...
  while (mesh_index < mesh_count) {
    ...
    eg_string_t *mesh_name = eg_string_format("unnamed mesh - %llu", mesh_index);
    ...
    if (gltf_mesh->name) {
      strcpy(model_mesh.name, gltf_mesh->name);
    } else if (gltf_node->name) {
      strcpy(model_mesh.name, gltf_node->name);
    } else {
      strcpy(model_mesh.name, eg_string_buffer(mesh_name));
    }

    eg_database_store_model_mesh(&model_mesh);
    ...
    while (prim_index < prim_count) {
      ...
      eg_string_t *prim_name = eg_string_format("unnamed primitive - %llu", prim_index);
      ...
      if (gltf_prim->extras.data) {
        ...
        if (stbj_any_error(&prim_extras)) {
          strcpy(mesh_primitive.name, eg_string_buffer(prim_name));
        } else {
          strcpy(mesh_primitive.name, name_buffer);
        }
      } else {
        strcpy(mesh_primitive.name, eg_string_buffer(prim_name));
      }

      eg_database_store_mesh_primitive(&mesh_primitive);
      ...
      while (attrib_index < attrib_count) {
        ...
        eg_string_t *attrib_name = eg_string_format("unnamed attribute - %llu", attrib_index);
        ...
        switch (gltf_accessor->type) {
          case cgltf_type_scalar: {
            ...
            value_data = (uint8_t *)eg_heap_alloc(sizeof(float) * value_size, 0, 0);
            break;
          }
        ...
        if (gltf_attrib->name) {
          strcpy(mesh_attribute.name, gltf_attrib->name);
        } else {
          strcpy(mesh_attribute.name, eg_string_buffer(attrib_name));
        }

        eg_database_store_mesh_attribute(&mesh_attribute);
        ...
        eg_database_store_attribute_buffer(&attribute_buffer);

        eg_heap_free(value_data);

        eg_string_destroy(attrib_name);

        attrib_index++;
      }

      eg_string_destroy(prim_name);

      prim_index++;
    }

    eg_string_destroy(mesh_name);
...
static void eg_importer_process_pipeline_vertex_input_bindings(eg_pipeline_asset_t *pipeline_asset, SpvReflectShaderModule *shader_module) {
  ...
  EG_SPIRV_CHECK(spvReflectEnumerateInputVariables(shader_module, &input_variable_count, 0));

  SpvReflectInterfaceVariable **input_variables = (SpvReflectInterfaceVariable **)eg_heap_alloc(sizeof(SpvReflectInterfaceVariable *) * input_variable_count, 0, 0);
  EG_SPIRV_CHECK(spvReflectEnumerateInputVariables(shader_module, &input_variable_count, input_variables));
  ...
  eg_heap_free(input_variables);
}
static void eg_importer_process_pipeline_descriptor_bindings(eg_pipeline_asset_t *pipeline_asset, SpvReflectShaderModule *shader_module, VkShaderStageFlags stage_flag) {
  uint32_t descriptor_binding_index = 0;
  uint32_t descriptor_binding_count = 0;

  EG_SPIRV_CHECK(spvReflectEnumerateDescriptorBindings(shader_module, &descriptor_binding_count, 0));

  SpvReflectDescriptorBinding **descriptor_bindings = (SpvReflectDescriptorBinding **)eg_heap_alloc(sizeof(SpvReflectDescriptorBinding *) * descriptor_binding_count, 0, 0);
  EG_SPIRV_CHECK(spvReflectEnumerateDescriptorBindings(shader_module, &descriptor_binding_count, descriptor_bindings));
  ...
  eg_heap_free(descriptor_bindings);
}
