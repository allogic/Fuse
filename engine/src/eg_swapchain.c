...
void eg_swapchain_create(eg_context_t *context, lb_swapchain_id_t swapchain_id) {
  ...
  swapchain->config = lb_database_load_swapchain_by_id(swapchain_id);

  swapchain->image_count = swapchain->config.image_count;
  swapchain->depth_format = swapchain->config.depth_format;

  swapchain->image_count = LB_MAX(swapchain->image_count, swapchain->context->surface_capabilities.minImageCount);
  swapchain->image_count = LB_MIN(swapchain->image_count, swapchain->context->surface_capabilities.maxImageCount);
  ...
  EG_VULKAN_CHECK(vkCreateSwapchainKHR(swapchain->context->device, &swapchain_create_info, 0, &swapchain->handle));
...
void eg_swapchain_destroy(eg_swapchain_t *swapchain) {
  ...
  lb_database_destroy_swapchain(&swapchain->config);
...
static void eg_swapchain_create_main_render_pass(eg_swapchain_t *swapchain) {
  ...
  EG_VULKAN_CHECK(vkCreateRenderPass(swapchain->context->device, &render_pass_create_info, 0, &swapchain->main_render_pass));
}
static void eg_swapchain_create_color_images(eg_swapchain_t *swapchain) {
  EG_VULKAN_CHECK(vkGetSwapchainImagesKHR(swapchain->context->device, swapchain->handle, (uint32_t *)&swapchain->image_count, swapchain->color_image));
  ...
  while (image_index < image_count) {
    ...
    EG_VULKAN_CHECK(vkCreateImageView(swapchain->context->device, &image_view_create_info, 0, &swapchain->color_image_view[image_index]));
...
static void eg_swapchain_create_depth_images(eg_swapchain_t *swapchain) {
  ...
  while (image_index < image_count) {
    ...
    EG_VULKAN_CHECK(vkCreateImage(swapchain->context->device, &image_create_info, 0, &swapchain->depth_image[image_index]));
    ...
    EG_VULKAN_CHECK(vkAllocateMemory(swapchain->context->device, &memory_allocate_info, 0, &swapchain->depth_device_memory[image_index]));
    EG_VULKAN_CHECK(vkBindImageMemory(swapchain->context->device, swapchain->depth_image[image_index], swapchain->depth_device_memory[image_index], 0));
    ...
    EG_VULKAN_CHECK(vkCreateImageView(swapchain->context->device, &image_view_create_info, 0, &swapchain->depth_image_view[image_index]));
...
static void eg_swapchain_create_frame_buffer(eg_swapchain_t *swapchain) {
  ...
  while (image_index < image_count) {
    ...
    EG_VULKAN_CHECK(vkCreateFramebuffer(swapchain->context->device, &frame_buffer_create_info, 0, &swapchain->frame_buffer[image_index]));
