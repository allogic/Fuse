...
static void eg_rigidbody_handle_angular_velocity(eg_context_t *context, eg_transform_t *transform, eg_rigidbody_t *rigidbody) {
  ...
  lb_quaternion_t qd = lb_quaternion_mul(qy, qp);

  transform->local_rotation = lb_quaternion_norm(lb_quaternion_mul(qd, transform->local_rotation));
  rigidbody->angular_velocity = lb_vector3_sub(rigidbody->angular_velocity, velocity_step);
}
