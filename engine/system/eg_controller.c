...
static void editor_controller_handle_rotation(context_t *context, editor_controller_t *editor_controller, transform_t *transform, rigidbody_t *rigidbody) {
  if (context_is_mouse_key_pressed(context, MOUSE_KEY_RIGHT)) {
    ...
    editor_controller->mouse_end = vector3_xyz((float)context->mouse_position_x, (float)context->mouse_position_y, 0.0F);
  ...
  vector3_t mouse_damping = vector3_muls(vector3_muls(editor_controller->mouse_delta, editor_controller->mouse_rotation_drag_damping), (float)context->delta_time);
