...
static void editor_controller_system_handle_position(editor_controller_t *editor_controller, transform_t *transform, rigidbody_t *rigidbody);
static void editor_controller_system_handle_rotation(editor_controller_t *editor_controller, transform_t *transform, rigidbody_t *rigidbody);

void editor_controller_system_update(ecs_iter_t *iter) {
  while (ecs_query_next(iter)) {
    ...
    while (entity_index < entity_count) {
      ...
      editor_controller_system_handle_position(&editor_controllers[entity_index], &transforms[entity_index], &rigidbodies[entity_index]);
      editor_controller_system_handle_rotation(&editor_controllers[entity_index], &transforms[entity_index], &rigidbodies[entity_index]);
...
static void editor_controller_system_handle_rotation(editor_controller_t *editor_controller, transform_t *transform, rigidbody_t *rigidbody) {
  if (context_is_mouse_key_pressed(MOUSE_KEY_RIGHT)) {
    ...
    editor_controller->mouse_begin = vector3_xyz((float)g_globals.context_mouse_position_x, (float)g_globals.context_mouse_position_y, 0.0F);
    ...
    editor_controller->mouse_end = vector3_xyz((float)g_globals.context_mouse_position_x, (float)g_globals.context_mouse_position_y, 0.0F);
    editor_controller->mouse_delta = vector3_sub(editor_controller->mouse_begin, editor_controller->mouse_end);

    float x_delta = editor_controller->mouse_delta.x * mouse_move_speed * (float)g_globals.context_delta_time;
    float y_delta = -editor_controller->mouse_delta.y * mouse_move_speed * (float)g_globals.context_delta_time;
    ...
    float p_delta = -editor_controller->mouse_delta.y * editor_controller->mouse_rotation_speed * (float)g_globals.context_delta_time;
    float y_delta = -editor_controller->mouse_delta.x * editor_controller->mouse_rotation_speed * (float)g_globals.context_delta_time;
  ...
  vector3_t mouse_damping = vector3_muls(vector3_muls(editor_controller->mouse_delta, editor_controller->mouse_rotation_drag_damping), (float)g_globals.context_delta_time);
