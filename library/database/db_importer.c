...
static void db_importer_process_vertex_input_binding_descriptions(int32_t pipeline_id, SpvReflectShaderModule *shader_module);
static void db_importer_process_vertex_input_attribute_descriptions(int32_t pipeline_id, SpvReflectShaderModule *shader_module);
static void db_importer_process_descriptor_pool_sizes(int32_t pipeline_id, SpvReflectShaderModule *shader_module);
...
void db_importer_insert_graphic_pipeline(char const *pipeline_name, char const *vertex_shader_file_path, char const *fragment_shader_file_path) {
  ...
  int32_t pipeline_id = db_get_pipeline_id();
  ...
  db_store_pipeline_config(pipeline_name, 0, 0);
  db_store_pipeline_resource(pipeline_id, vertex_shader, vertex_shader_size, fragment_shader, fragment_shader_size, 0, 0);

  db_importer_process_vertex_input_binding_descriptions(pipeline_id, &reflect_vertex_shader_module);
  db_importer_process_vertex_input_attribute_descriptions(pipeline_id, &reflect_vertex_shader_module);
  db_importer_process_descriptor_pool_sizes(pipeline_id, &reflect_vertex_shader_module);
  db_importer_process_descriptor_pool_sizes(pipeline_id, &reflect_fragment_shader_module);
...
void db_importer_insert_compute_pipeline(char const *pipeline_name, char const *compute_shader_file_path) {
  ...
  db_store_pipeline_config(pipeline_name, 1, 0);
  db_store_pipeline_resource(pipeline_id, 0, 0, 0, 0, compute_shader, compute_shader_size);

  db_importer_process_descriptor_pool_sizes(pipeline_id, &reflect_compute_shader_module);
...
static void db_importer_process_vertex_input_binding_descriptions(int32_t pipeline_id, SpvReflectShaderModule *shader_module) {
  ...
  while (input_variable_index < input_variable_count) {
    ...
    db_store_pipeline_vertex_input_binding_description(pipeline_id, input_variable->name, 0, 0, VK_VERTEX_INPUT_RATE_VERTEX);
...
static void db_importer_process_vertex_input_attribute_descriptions(int32_t pipeline_id, SpvReflectShaderModule *shader_module) {
  ...
  while (input_variable_index < input_variable_count) {
    ...
    db_store_pipeline_vertex_input_attribute_description(pipeline_id, input_variable->name, input_variable->location, 0, input_variable->format, byte_offset);
...
static void db_importer_process_descriptor_pool_sizes(int32_t pipeline_id, SpvReflectShaderModule *shader_module) {
  ...
  while (descriptor_binding_index < descriptor_binding_count) {
    ...
    db_store_pipeline_descriptor_pool_size(pipeline_id, descriptor_binding->descriptor_type, descriptor_binding->count);
...
static void db_importer_process_descriptor_set_layout_bindings(int32_t pipeline_id, SpvReflectShaderModule *shader_module, VkShaderStageFlags stage_flag) {
  ...
  while (descriptor_binding_index < descriptor_binding_count) {
    ...
    db_store_pipeline_descriptor_set_layout_bindings(pipeline_id, descriptor_binding->name, descriptor_binding->binding, descriptor_binding->descriptor_type, 1, stage_flag, 1);
