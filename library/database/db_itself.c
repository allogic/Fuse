...
static int32_t db_get_next_sequence_index_by_name(char const *table_name);
...
vector_t db_load_pipeline_descriptor_set_layout_bindings_by_id(int32_t pipeline_id) {
  ...
  string_appendf(&sql, "ON PA.ID = PDSLB.PIPELINE_ID\n");
  string_appendf(&sql, "WHERE PA.ID = %d\n", pipeline_id);
...
int32_t db_store_pipeline_asset(char const *pipeline_name, uint32_t pipeline_type, uint32_t pipeline_link_index, uint32_t pipeline_auto_create) {
  ...
  return db_get_next_sequence_index_by_name("PIPELINE_ASSETS");
}
int32_t db_store_pipeline_resource(int32_t pipeline_id, uint8_t *vertex_shader, uint64_t vertex_shader_size, uint8_t *fragment_shader, uint64_t fragment_shader_size, uint8_t *compute_shader, uint64_t compute_shader_size) {
  ...
  return db_get_next_sequence_index_by_name("PIPELINE_RESOURCES");
}
int32_t db_store_pipeline_vertex_input_binding(int32_t pipeline_id, char const *binding_name, uint32_t binding, uint32_t location, uint32_t stride, uint32_t format, uint32_t offset, uint32_t input_rate) {
  ...
  return db_get_next_sequence_index_by_name("PIPELINE_VERTEX_INPUT_BINDINGS");
}
int32_t db_store_pipeline_descriptor_set_layout_binding(int32_t pipeline_id, char const *binding_name, uint32_t binding, uint32_t descriptor_type, uint32_t descriptor_count, uint32_t stage_flags, uint8_t auto_buffer) {
  ...
  string_appendf(&sql, "PIPELINE_ID = PIPELINE_DESCRIPTOR_SET_LAYOUT_BINDINGS.PIPELINE_ID,\n");
  string_appendf(&sql, "BINDING = PIPELINE_DESCRIPTOR_SET_LAYOUT_BINDINGS.BINDING,\n");
  string_appendf(&sql, "STAGE_FLAGS = PIPELINE_DESCRIPTOR_SET_LAYOUT_BINDINGS.STAGE_FLAGS | EXCLUDED.STAGE_FLAGS\n");
  ...
  return db_get_next_sequence_index_by_name("PIPELINE_DESCRIPTOR_SET_LAYOUT_BINDINGS");
}
...
void db_destroy_pipeline_descriptor_set_layout_bindings(vector_t *descriptor_set_layout_bindings) {
  ...
  uint64_t descriptor_set_layout_count = vector_count(descriptor_set_layout_bindings);

  while (descriptor_set_layout_index < descriptor_set_layout_count) {
    pipeline_descriptor_set_layout_binding_t *pipeline_descriptor_set_layout_binding = (pipeline_descriptor_set_layout_binding_t *)vector_at(descriptor_set_layout_bindings, descriptor_set_layout_index);

    string_destroy(&pipeline_descriptor_set_layout_binding->binding_name);
