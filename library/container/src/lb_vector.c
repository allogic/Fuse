...
lb_vector_t *lb_vector_create(uint64_t value_size) {
  lb_vector_t *vector = (lb_vector_t *)lb_heap_alloc(sizeof(lb_vector_t), 1, 0);
...
uint8_t lb_vector_equal(lb_vector_t *vector, lb_vector_t *reference) {
  ...
  not_equal |= memcmp(vector->buffer, reference->buffer, LB_MIN(vector->buffer_size, reference->buffer_size));
...
void lb_vector_remove(lb_vector_t *vector, void const *value) {
  if (value) {
    ...
    lb_vector_swap(vector, buffer_index, vector->buffer_count - 1);
