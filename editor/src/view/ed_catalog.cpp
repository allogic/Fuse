...
ed_catalog_view_t *ed_catalog_view_create(eg_context_t *context, eg_asset_type_t asset_type) {
  ed_catalog_view_t *catalog = (ed_catalog_view_t *)eg_heap_alloc(sizeof(ed_catalog_view_t), 1, 0);
  ...
  switch (catalog->asset_type) {
    case EG_ASSET_TYPE_SWAPCHAIN: {
      ...
      catalog->assets = eg_database_load_all_swapchain_assets();

      break;
    }
    case EG_ASSET_TYPE_RENDERER: {

      catalog->assets = eg_database_load_all_renderer_assets();

      break;
    }
    case EG_ASSET_TYPE_PIPELINE: {

      catalog->assets = eg_database_load_all_pipeline_assets();

      break;
    }
    case EG_ASSET_TYPE_MODEL: {

      catalog->assets = eg_database_load_all_model_assets();

      break;
    }
    case EG_ASSET_TYPE_SCENE: {

      catalog->assets = eg_database_load_all_scene_assets();
...
void ed_catalog_view_refresh(ed_catalog_view_t *catalog) {
  switch (catalog->asset_type) {
    case EG_ASSET_TYPE_SWAPCHAIN: {
      ...
      eg_database_destroy_swapchain_assets(catalog->assets);

      break;
    }
    case EG_ASSET_TYPE_RENDERER: {

      eg_database_destroy_renderer_assets(catalog->assets);

      break;
    }
    case EG_ASSET_TYPE_PIPELINE: {

      eg_database_destroy_pipeline_assets(catalog->assets);

      break;
    }
    case EG_ASSET_TYPE_MODEL: {

      eg_database_destroy_model_assets(catalog->assets);

      break;
    }
    case EG_ASSET_TYPE_SCENE: {

      eg_database_destroy_scene_assets(catalog->assets);
...
void ed_catalog_view_draw(ed_catalog_view_t *catalog) {
  ...
  if (ImGui::BeginTable("##AssetTable", 2, table_flags)) {
    ...
    uint64_t asset_count = eg_vector_count(catalog->assets);
...
void ed_catalog_view_destroy(ed_catalog_view_t *catalog) {
  ...
  eg_heap_free(catalog);
}

static void ed_catalog_view_draw_swapchain(ed_catalog_view_t *catalog, uint64_t asset_index) {
  eg_swapchain_asset_t *swapchain_asset = (eg_swapchain_asset_t *)eg_vector_at(catalog->assets, asset_index);
...
static void ed_catalog_view_draw_renderer(ed_catalog_view_t *catalog, uint64_t asset_index) {
  eg_renderer_asset_t *renderer_asset = (eg_renderer_asset_t *)eg_vector_at(catalog->assets, asset_index);
...
static void ed_catalog_view_draw_pipeline(ed_catalog_view_t *catalog, uint64_t asset_index) {
  eg_pipeline_asset_t *pipeline_asset = (eg_pipeline_asset_t *)eg_vector_at(catalog->assets, asset_index);
...
static void ed_catalog_view_draw_model(ed_catalog_view_t *catalog, uint64_t asset_index) {
  eg_model_asset_t *model_asset = (eg_model_asset_t *)eg_vector_at(catalog->assets, asset_index);
...
static void ed_catalog_view_draw_scene(ed_catalog_view_t *catalog, uint64_t asset_index) {
  eg_scene_asset_t *scene_asset = (eg_scene_asset_t *)eg_vector_at(catalog->assets, asset_index);
